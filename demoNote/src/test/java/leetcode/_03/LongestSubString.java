...
public class LongestSubString {
    ...
    public static int lengthOfLongestSubstring(String s) {
        /**
         * 先找出重复字符，并按字符分组，
         * 规定下标范围为(-1 ~ len)根据下标计算长度，由最长到最短
         * 检查下标范围内是否有重复字符
         *      如果有，检查下一个
         *      如果没有，返回该长度
         *
         * 如 "abcabcbb"
         * 得到重复字符及其下标： a -> [-1,0,3,8], b -> [-1,1,4,6,7,8], c -> [-1,2,5,8]
         * 得到下标距离
         * 5 a(3,8)
         * 3 a(0,3),b(1,4),c(2,5),c(5,8),c(-1,2)
         * 2 b(4,6),b(-1,2)
         * 1 a(-1,0),b(6,7),b(7,8)
         * 检查 a(3,8)中有b(4,6,7),下一个
         * 检查 a(0,3)中没有重复字符，返回3
         */
        ...
        class Demo implements Comparable<Demo> {
            ...
            public int compareTo(Demo o) {
                return o.getLen() - this.getLen();
            }
        }

        Map<Character,List<Integer>> newMap = new HashMap<>();
        Iterator<Map.Entry<Character, List<Integer>>> iterator1 = map.entrySet().iterator();
        while (iterator1.hasNext()) {
            Map.Entry<Character, List<Integer>> next = iterator1.next();
        ...
        while(iterator.hasNext()){
            ...
            integers = map.get(next);
            Demo demo;
            int size = integers.size();
            for (int i = 1; i < size; i++) {
                demo = new Demo();
                demo.setCh(next);
                demo.setLeftIndex(integers.get(i-1));
                if(i == size - 1){
                    demo.setRightIndex(length);
                } else {
                    demo.setRightIndex(integers.get(i+1));
                }
                demo.setLen(demo.getRightIndex() - demo.getLeftIndex() - 1);
                list.add(demo);
            }
        }
        ...
        int maxProbilyLen = demo.getLen();
        do {
            Iterator<Character> keyIterator;
            out1 : for (int i = 0; i <= length - maxProbilyLen; i++) {
                keyIterator = characters.iterator();
                while (keyIterator.hasNext()) {
                    Character next = keyIterator.next();
                    List<Integer> integers1 = map.get(next);
                    int num = 0;
                    for (int i1 = 0; i1 < integers1.size(); i1++) {
                        Integer integer = integers1.get(i1);
                        if (integer >= i && integer <= i + maxProbilyLen - 1){
                            num++;
                        }
                    }
                    if(num > 1){
                ...
                return maxProbilyLen;
            }
        } while (--maxProbilyLen >= 0);


//        Demo demo;
//        Iterator<Character> keyIterator;
//        out1 : for (int i = 0; i < list.size(); i++) {
//            keyIterator = characters.iterator();
//            demo = list.get(i);
//            while (keyIterator.hasNext()) {
//                Character next = keyIterator.next();
...
//                for (int i1 = 0; i1 < integers1.size(); i1++) {
//                    Integer integer = integers1.get(i1);
//                    if (integer > demo.getLeftIndex() && integer < demo.getRightIndex()){
//                        num++;
//                    }
//                }
//                if(num > 1){

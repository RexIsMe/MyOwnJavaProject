package com.atguigu.teacher.member.service

import com.alibaba.fastjson.JSONObject
import com.atguigu.teacher.util.ParseJsonData
...
object EtlDataService {
  ...
  def etlMemberRegtypeLog(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/atguigu/ods/memberRegtype.log")
      .filter(item => {
  ...
  def etlMemberLog(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._ //隐式转换
    ssc.textFile("/user/atguigu/ods/member.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
  ...
  def etlBaseAdLog(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    val result = ssc.textFile("/user/atguigu/ods/baseadlog.log")
      ...
      //先用mapPartitions而不直接用map，可以提高效率
      //但是也要注意，缺点是把整个分区的数据放到一个迭代器中，可能有OOM的风险
      .mapPartitions(partition => {
        //对分区中的每个元素做处理
        partition.map(item => {
          val jsonObject = ParseJsonData.getJsonData(item)
      ...
      //rdd转成DataFrame，因为DF有现成的算子保存到hive中
      .toDF()
      //作用：减少分区避免生成过多小文件
      .coalesce(1)
      .write.mode(SaveMode.Append)
      /**
      * insertInto和saveAsTable比较
        * 注意前面tuple中元素的书序和表中列的顺序一致
      */
  ...
  def etlBaseWebSiteLog(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._ //隐式转换
    ssc.textFile("/user/atguigu/ods/baswewebsite.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partition => {
      partition.map(item => {
